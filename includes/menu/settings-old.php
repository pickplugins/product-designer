...
if(empty($_POST['product_designer_hidden'])){
    ...
    if(wp_verify_nonce( $nonce, 'product_designer_nonce' ) && sanitize_text_field($_POST['product_designer_hidden']) == 'Y') {

        //Form data sent

        $product_designer_page_id = isset($_POST['product_designer_page_id']) ? sanitize_text_field($_POST['product_designer_page_id']) : '';
        update_option('product_designer_page_id', $product_designer_page_id);

        $product_designer_posts_per_page = isset($_POST['product_designer_posts_per_page']) ? sanitize_text_field($_POST['product_designer_posts_per_page']) : '';
...
<div class="wrap">
	<?php
    echo "<h2>".sprintf(__('%s Settings',  'product-designer'), product_designer_plugin_name)."</h2>";
                    ...
                    <div class="faq">
			            ...
			            foreach($faq as $faq_data){
				            ...
				            foreach($items as $item){
					            ...
					            echo '<li class="item">';
					            echo '<a href="'.esc_url_raw($item['answer_url']).'"><i class="fa fa-question-circle-o" aria-hidden="true"></i> '.$item['question'].'</a>';
